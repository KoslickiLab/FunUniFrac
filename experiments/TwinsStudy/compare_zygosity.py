...
for scale in ["1000", "10000"]:
    for method in ["AAI", "motifs", "uniform"]:
        for kSize in [5, 7, 11, 15]:
            for approach in ["weighted", "unweighted"]:
                #scale = 1000
                #method = "AAI"
                #kSize = 15
                #approach = "unweighted"
                ...
                if approach == "unweighted":
                    ...
                    to_insert = ""
                pairwise_dists_file = f"output/merged_pw_fu_{to_insert}{method}_scale_{scale}_k_{kSize}_f_unique_weighted.np.npy"
                ...
                # also do Mood's median test
                from scipy.stats import median_test
                mp01 = median_test(unifrac_of_DZ_twin_pairs, unifrac_of_MZ_twin_pairs)[1]
                mp02 = median_test(unifrac_of_DZ_twin_pairs, unifrac_of_unrelated_pairs)[1]
                mp12 = median_test(unifrac_of_MZ_twin_pairs, unifrac_of_unrelated_pairs)[1]
                print(mp01)
                print(mp02)
                print(mp12)
                ...
                # add a title
                if approach == "unweighted":
                    ax.set_title(f"Unweighted UniFrac, Tree: {method}, kSize: {kSize}, scale: {scale}")
                else:
                    ax.set_title(f"Weighted UniFrac, Tree: {method}, kSize: {kSize}, scale: {scale}")
                max_val = max([max(unifrac_of_DZ_twin_pairs), max(unifrac_of_MZ_twin_pairs), max(unifrac_of_unrelated_pairs)])
                x1, x2 = 0, 1
                y, h, col = max_val*(1 + 0.2), 0.01, 'k'
                ...
                plt.savefig(f"output/zygosity_boxplot_paired_method_{method}_ksize_{kSize}_{approach}_scale_"
                            f"{scale}.png",
                            dpi=300,
                            bbox_inches='tight')
                plt.show()
