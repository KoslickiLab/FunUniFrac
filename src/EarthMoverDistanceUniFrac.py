...
class EarthMoverDistanceUniFracAbstract(abc.ABC):
    ...
    @abc.abstractmethod
    def functional_profile_to_EMDU_vector(self,
    ...
    @abc.abstractmethod
    def EMDUnifrac_weighted_flow(self,
                                 Tint, 
                                 lint, 
                                 nodes_in_order, 
                                 P, 
    ...
    # This will return the EMDUnifrac distance only
    @abc.abstractmethod
    def EMDUnifrac_unweighted(Tint, 
                              lint, 
                              nodes_in_order, 
                              P, 
                              Q):
    ...
    @abc.abstractmethod
    def EMDUnifrac_unweighted_flow(Tint, 
                                   lint, 
                                   nodes_in_order, 
                                   P, 
                                   Q):
    ...
    @abc.abstractmethod
    def plot_diffab(nodes_in_order, 
                    diffab, 
                    P_label, 
                    Q_label, 
                    plot_zeros=True, 
...
class EarthMoverDistanceUniFracSolver(EarthMoverDistanceUniFracAbstract):
    ...
    def functional_profile_to_EMDU_vector(self, functional_profile_file, EMDU_index_2_node, abundance_key='median_abund', normalize=True):
    ...
    def EMDUnifrac_weighted_flow(self, Tint, lint, nodes_in_order, P, Q):
    ...
    # This will return the EMDUnifrac distance only
    def EMDUnifrac_weighted(self, Tint, lint, nodes_in_order, P, Q):
        num_nodes = len(nodes_in_order)
        Z = 0
        diffab = dict()
        partial_sums = P - Q
    ...
    def EMDUnifrac_unweighted(self, Tint, lint, nodes_in_order, P, Q):
        num_nodes = len(nodes_in_order)
        Z = 0
        diffab = dict()
        for i in range(num_nodes):
    ...
    def EMDUnifrac_unweighted_flow(self, Tint, lint, nodes_in_order, P, Q):
        num_nodes = len(nodes_in_order)
        F = dict()
        G = dict()
        diffab = dict()
    ...
    def push_up_L2(self, P, Tint, lint, nodes_in_order):
    ...
    def push_up_L1(self, P, Tint, lint, nodes_in_order):
        P_pushed = copy.deepcopy(P)
        for i in range(len(nodes_in_order)-1):
            if lint[i, Tint[i]] == 0:
                lint[i, Tint[i]] = epsilon
    ...
    def EMD_L1_on_pushed(self, P, Q):
        return np.sum(np.abs(P-Q))


    def EMD_L1_and_diffab_on_pushed(self, P, Q):
        Z = np.sum(np.abs(P-Q))
        diffab = P-Q
        return Z, diffab


    def EMD_L2_on_pushed(self, P, Q):
        return np.sum(np.abs(P-Q)**2)


    def EMD_L2_and_diffab_on_pushed(self, P, Q):
