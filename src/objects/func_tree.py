...
class FuncTree:
    def __init__(self, G) -> None:
        ...
        self._current_subtree = None
        self._current_subtree_undirected = None
    
    @property
    def current_subtree(self):
        self.check_subtree_valid()
        return self._current_subtree
    
    @property
    def current_subtree_undirected(self):
        self.check_subtree_valid()
        return self._current_subtree_undirected
    
    @property
    def basis(self):
        self.check_subtree_valid()
        # set the basis for the tree, which is an ordering of the edges. I'll identify each edge by its terminal node
        basis = [x for x in self._current_subtree.nodes()]
        return basis
    ...
    def check_subtree_valid(self):
        if self._current_classification is None:
            raise Exception("classification is not set. subtree is not defined")
    
    def apply_classification(self, classification):
        G = self._G
        # get the descendants of the brite
        descendants = self.get_descendants(G, classification)
        ...
        self._current_subtree = G.subgraph(descendants)
        self._current_subtree_undirected = G.to_undirected()
    
    def get_descendants(self, G, node):
        """
