...
class KeggTree:
    ...
    def get_siblings(self, node):
        ...
        if len(parents) > 0:
            for parent in parents:
                siblings.update(self.tree.successors(parent))
            siblings.discard(node)  # Remove the node itself
        else:
    ...
    def fill_leaf_pairs_distances(self, pw_dist_file, label_file):
        ...
        last_depth_index = len(self.nodes_by_depth) - 1

        # Create a cache for parent information
        parent_cache = {}

        def get_cached_parent(node):
            if node not in parent_cache:
                parent = self.get_parent(node)
                while parent.startswith('dummy'):
                    parent = self.get_parent(parent)
                parent_cache[node] = parent
            return parent_cache[node]

        for (a, b) in self.needed_pairs[last_depth_index]:
            p_a = a if not a.startswith('dummy') else get_cached_parent(a)
            p_b = b if not b.startswith('dummy') else get_cached_parent(b)

            a_index = label_pos[p_a]
            b_index = label_pos[p_b]
            self.needed_pairs[last_depth_index][(a, b)] = pw_dist[a_index][b_index]
    ...
    def solve_branch_lengths(self, edge_length_solutions, level):
        ...
        '''
        while level >= 1:
            if level == 1 and len(self.needed_pairs[1]) == 1:
                ...
                for node in self.partners[level]:
                    ...
                    if self.partners[level][node] == 0:
                        ...
                        sib, another = self.partners[level][node]
    ...
    def update_needed_pairs(self, level, edge_length_solutions):
        ...
        needed_pairs_level = self.needed_pairs[level]
        needed_pairs_level1 = self.needed_pairs[level + 1]

        for (a, b) in needed_pairs_level:
            a_child = self.get_child(a)
            b_child = self.get_child(b)

            # Precalculate frequently used values
            a_child_b_child_dist = needed_pairs_level1[(a_child, b_child)]
            a_a_child_dist = edge_length_solutions[(a, a_child)]
            b_b_child_dist = edge_length_solutions[(b, b_child)]

            # Calculate and assign
            needed_pairs_level[(a, b)] = a_child_b_child_dist - a_a_child_dist - b_b_child_dist
...
def post_process(edge_length_solution, kegg_tree):
    ...
    for i in range(len(kegg_tree.nodes_by_depth)):
        for node in kegg_tree.nodes_by_depth[i]:
            successors = list(kegg_tree.tree.successors(node))
            if len(successors) == 1:
                child = successors[0]
                if not child.startswith('dummy') and edge_length_solution[(node, child)] == 0:
                    nodes_on_the_way = [node]
                    cur_node = child
                    while len(list(kegg_tree.tree.successors(cur_node))) == 1 and not cur_node.startswith('dummy'):
                        nodes_on_the_way.append(cur_node)
                        cur_node = list(kegg_tree.tree.successors(cur_node))[0]
                    ave_edge_length = edge_length_solution[(node, child)] / len(nodes_on_the_way)
                    for n in nodes_on_the_way:
                        edge_length_solution[(node, n)] = ave_edge_length
