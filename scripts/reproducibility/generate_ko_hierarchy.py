...
class KEGG_KO_Extraction:
    ...
    def __get_brite_table(self):
        ...
        link = f"{self.KEGG_api_link}/list/brite"
    ...
    def run(self, brite_id):

        substree_dict = dict()
        print(f"Processing brite id {brite_id}", flush=True)
        check_link = f"{self.KEGG_api_link}/get/{brite_id}"
        ...
        if m is None:
            ...
            return
        link = f"{self.KEGG_api_link}/get/{brite_id}/json"
        json_res = requests.get(link)
        if json_res.status_code == 200:
            temp_dict = self.organize_hierarchy(json_res.json(), regex='K\d{5}')
...
def main(args):
    ...
    for brite_id in brite_list:

        substree_dict = ko_extractor.run(brite_id)
        ...
        for ko_id, hierarchy_list in substree_dict.items():
            for item in hierarchy_list:
                temp_list = [ko_extractor.id_mapping.get(x,x) for x in item.split('|')[:-1]]
