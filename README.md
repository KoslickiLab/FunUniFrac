...
All required resources are shared by Onedrive currently. [Please check here.](https://pennstateoffice365-my.sharepoint.com/:f:/g/personal/akp6031_psu_edu/Ele7H9T9yuRLuAdBY_WDS90BuTCYwk1z6VYqkmdJj9ATnQ?e=ccM4gy)
...
```
The resulting matrix will have a name such as: `fununifrac_A.npz` and will be a 
...
large enough. After a bunch of iterates, the average is then taken. 
...
is based on is obtained from the KEGG database.  

The KEGG Orthology (KO) hierarchy tree was donwloaded using 
[KEGG API](https://www.kegg.jp/kegg/rest/keggapi.html#list). The [KEGG BRITE Database](https://www.genome.jp/kegg/brite.html) 
is a collection of hierarchical classification system in which [ko00001](https://www.genome.jp/brite/ko00001) is the KO
hierarchy. Therefore, we utilized the [get](https://www.kegg.jp/kegg/rest/keggapi.html#get) operation of KEGG API to 
access the [KO hierarchy](https://rest.kegg.jp/get/br:ko00001). For each KO leaf node in this hierarchy tree, we get its
corresponding lineage. For instance, the lineage of KO `K03382` is "09100 Metabolism; 09111 Xenobiotics biodegradation 
and metabolism; 00791 Atrazine degradation; K03382  atzB; hydroxydechloroatrazine ethylaminohydrolase". With this 
lineage information, we save the KO hierarchical tree into a file `kegg_ko00001_edges.txt` with an edge list format.  

Currently, to keep things simple, we are just using sub**trees** of the KEGG database. 
...
For a more detailed description of the process, see [extraction repo](https://github.com/KoslickiLab/KEGG_data_extraction).
In particular:
...
With the KOs sketched, we can also compute the pairwise distances among the KOs, which correspond to the leaf nodes
of the underlying tree obtained from KEGG.
